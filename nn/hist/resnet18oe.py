...
class Resnet18e(object):
        ...
        with tf.variable_scope('{}encoder'.format(prefix), reuse=tf.AUTO_REUSE) as scope:
            ...
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope('conv1'):
                    ...
                    x = self._bn(x)
                    x = self._activate(x, type='elu',name='elu1')
    ...
    def _residual_blocks(self, x, out_channel, stride=1, name='unit'):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE) as scope:
            ...
            x = x + short_cut
            x = self._activate(x, type='elu', name='elu2')
            return x
    ...
    def _residual_block_first(self, x, out_channel, stride, name='unit'):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE) as scope:
            ...
            if in_channel == out_channel:
                ...
                short_cut = self._conv(x, 1, out_channel, stride, name='shortcut')
            '''
            short_cut = self._conv(short_cut, 3, out_channel, stride, name='conv_s')
            short_cut = self._bn(short_cut, name='BatchNorm_s')
            short_cut = self._activate(short_cut, type='elu', name='elu_s')
    ...
    def _residual_block(self, x, name='unit'):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE) as scope:
            ...
            short_cut = x
            short_cut = self._conv(short_cut, 3, num_channel, 1, name='conv_s')
            short_cut = self._bn(short_cut, name='BatchNorm_s')
            short_cut = self._activate(short_cut, type='elu', name='elu_s')
